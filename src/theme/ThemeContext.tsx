      ...
      try {
        ...
        if (savedThemeId && themes[savedThemeId]) {
          ...
          if (savedThemeId === "xmb_wave" && theme.assets?.startupAudio && !hasPlayedAudio.current) {
            const isWebPath = theme.assets.startupAudio.startsWith("/");
            const assetUrl = isWebPath ? theme.assets.startupAudio : convertFileSrc(theme.assets.startupAudio);
            
            console.log(`[ThemeContext] Attempting to play startup audio: ${theme.assets.startupAudio}`);
            console.log(`[ThemeContext] Final URL: ${assetUrl}`);
